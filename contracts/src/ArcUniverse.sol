...
contract ArcUniverse {
    ...
    struct Universe {
        ...
        /* 铸造那一刻，合约用 blockhash(blockNumber) 亲自核对过这个哈希吗？
           EVM 只能回看最近 256 个块（Arc 上约 128 秒），所以：
    ...
    /* ---- 铸造总量上限（specs/arcbang-v1.md §3.1）----
       **1,387 枚** —— 宇宙年龄 137.87 亿年（Planck 2018: 13.787 Gyr），
       ...
       早先那套 170 万枚的量是配着「铸造发币」设计的；没有代币之后，
       那么大的量等于没有稀缺，所以这里不照搬。
    ...
    /* ============================================================ 引爆（mint）
       ...
       **只有 bangSigned() 一条路。** v4 那个不带签名的 bang() 已删除，理由有两条：
       ...
       2. 它是个真实的洞（在早先那版合约上已经被人走过）：bang() 按扁平价收费，
          而分档表里 S 档标价更高 —— 而 _bang 会写 tokenOfHash。
          **按最低价就能永久抢占一个 S 档区块，让它再也没人能铸**，
