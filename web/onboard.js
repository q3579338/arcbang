...
(function (root, doc) {
  ...
  /* ============================================================ 三步引导 */
  /* BTC 站（bang.satloot.com，specs/btcbang-v1.md）：第 1 步的链名与节奏换成比特币口径，
     其余两步与 bnb 站一字不差。站标识与 nav.js siteOf 同源：config.js 在本文件之前注入。 */
  var IS_BTC = (root.BNBBANG_CONFIG && root.BNBBANG_CONFIG.site === 'btc') || root.BNBBANG_SITE === 'btc';
  /* ARCBANG（arcbang / specs/arcbang-v1.md）：结构照抄 BTC 那条先例 —— 只换第 1 步的链名
     与节奏，其余两步与 bnb 站一字不差。Arc 上没有代币，第 1 步里也不许出现 BNB / BANG。 */
  var IS_ARC = (root.BNBBANG_CONFIG && root.BNBBANG_CONFIG.site === 'arc') || root.BNBBANG_SITE === 'arc';
  var STEPS = [
    {
      k: '第 1 步',
      t: '一个区块哈希，就是一套物理定律',
      p: IS_BTC
        ? '比特币每 10 分钟出一个区块，全世界同时看见同一串 64 位哈希。这串数字被拆成 20 个创世参数 —— ' +
          '电磁力有多强、暗能量有多少、空间有几个维度……全由它决定。' +
          '你不需要懂这些参数：换一个区块，就是换一套物理定律。'
        : IS_ARC
        ? 'Arc 链每 0.5 秒吐出一个区块，每个区块带一串 64 位哈希。这串数字被拆成 20 个创世参数 —— ' +
          '电磁力有多强、暗能量有多少、空间有几个维度……全由它决定。' +
          '你不需要懂这些参数：换一个区块，就是换一套物理定律。'
        : 'BNB 链每隔几秒吐出一个区块，每个区块带一串 64 位哈希。这串数字被拆成 20 个创世参数 —— ' +
          '电磁力有多强、暗能量有多少、空间有几个维度……全由它决定。' +
          '你不需要懂这些参数：换一个区块，就是换一套物理定律。',
      d: IS_BTC ? '00000000…7661　→　20 个物理常数' : '0x9a00…6478　→　20 个物理常数'
    },
    {
      k: '第 2 步',
      t: '引爆，看它长成什么样',
    ...
    {
      k: '第 3 步',
      /* ARCBANG：拯救系统整套下线，「救」是那套话术的词，沙盒在这个站上叫「调参沙盒」，
         而且纯粹是玩法 —— 推出来的参数不上链、也铸不了。其余两站一字不动。 */
      t: IS_ARC ? '死掉的宇宙，能推活' : '死掉的宇宙，能救',
      p: IS_ARC
        ? '结局不好别扔。进「调参沙盒」，照着提示一格一格地推参数，右边的仪表会跟着动 ——' +
          '结局标签常常十几格才跳一次，动的是仪表。实测 83% 的死宇宙能推活，中位 4 步。' +
          '沙盒不花钱、不上链、可撤销，随便试。'
        : '结局不好别扔。进「干预沙盒」，照着提示一格一格地推参数，右边的仪表会跟着动 ——' +
          '结局标签常常十几格才跳一次，动的是仪表。实测 83% 的死宇宙能救活，中位 4 步。' +
          '沙盒不花钱、不上链、可撤销，随便试。',
